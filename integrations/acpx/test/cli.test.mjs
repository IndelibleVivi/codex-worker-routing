...
import { loadBinding,paths } from '../src/state.mjs';
import { fixture,fakeAcpx,sleep } from './helpers.mjs';
...
test('two commands cannot write the same workspace concurrently',async t=>{
 const f=await setup(t,{delay:350});const p=main(args(f),f.deps);
 while(!f.api.calls.some(c=>c[0]==='start'))await sleep(5);
 await assert.rejects(main(args(f),f.deps),{code:'BUSY_OR_UNRECONCILED'});assert.equal(await p,0);
});
