...
import { executeTurn, closeBinding, validateResume } from '../src/engine.mjs';
...
test('new work creates one persistent session; canonical result and cleanup are separate',async t=>{
 ...
 assert.equal(r.adapter_reported_session_usage,null);assert.equal(r.provider_identity_verified,false);
...
test('agent-reported usage is preserved as session usage, never fabricated',async t=>{
 const usage={cumulative:{inputTokens:9},cost:{amount:0,currency:'USD'}};const f=await setup(t,{usage});const r=await run(f);assert.deepEqual(r.adapter_reported_session_usage,usage);assert.equal(r.advertised_model,null);
});
